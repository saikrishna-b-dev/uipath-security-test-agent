...
class ReportAgent:
    ...
    def _render_html(self, run: OrchestratorRun) -> str:  # noqa: C901
        ...
        findings_rows = "".join(
            f'<tr><td>{f.name}</td><td>{badge(f.severity, 1)}</td>'
            f'<td style="max-width:300px;overflow:hidden;text-overflow:ellipsis">{f.url}</td>'
            f'<td>CWE-{f.cwe_id or "N/A"}</td></tr>'
            for f in sorted(run.security_findings, key=lambda x: list(SeverityLevel).index(x.severity))
        )

        heal_rows = "".join(
            f'<tr>'
            f'<td>{v.repair.diagnosis.test_failure.test_case_name}</td>'
            f'<td>{v.repair.diagnosis.failure_category}</td>'
            f'<td>{v.repair.action_taken.value}</td>'
            f'<td style="color:{"#16a34a" if v.re_run_passed else "#dc2626"};font-weight:bold">'
            f'{"✅ HEALED" if v.re_run_passed else "❌ FAILED"}</td>'
            f'</tr>'
            for v in run.validations
        )
...
{"<p style='color:#64748b;font-style:italic'>No security findings this cycle.</p>" if not run.security_findings else f"""
<table>
  <thead><tr><th>Finding</th><th>Severity</th><th>URL</th><th>CWE</th></tr></thead>
  <tbody>{findings_rows}</tbody>
</table>"""}

<h2>🔧 Self-Healing Results</h2>
{"<p style='color:#64748b;font-style:italic'>No test failures detected this cycle.</p>" if not run.validations else f"""
<table>
  <thead><tr><th>Test Case</th><th>Failure Category</th><th>Healing Action</th><th>Outcome</th></tr></thead>
